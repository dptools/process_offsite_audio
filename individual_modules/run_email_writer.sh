...
python "$repo_root"/individual_modules/functions_called/offsite_audio_email_write.py "$data_root" "$study" "$repo_root"/audio_lab_email_body.txt "$repo_root"/audio_transcribeme_email_body.txt
...
for p in *; do
	...
	if [[ ! -d $p/offsite_interview/processed/pending_audio ]]; then
		continue
	fi
	...
	if [ ! -d audio_to_send ] && [ -z "$(ls -A decrypted_audio)" ] && [ -z "$(ls -A pending_audio)" ]; then
	...
	if [[ -d audio_to_send ]]; then
		...
		echo "Participant ${p} Interviews Remaining to be Pushed (${num_failed} Total) - " >> "$repo_root"/audio_lab_email_body.txt
	...
	echo "" >> "$repo_root"/audio_lab_email_body.txt # want a blank line in between the different patient headers
	echo "Participant ${p} Interviews Pushed to TranscribeMe (${num_pushed} Total) - " >> "$repo_root"/audio_lab_email_body.txt
	...
	echo "" >> "$repo_root"/audio_lab_email_body.txt # want a blank line in between the different patient headers
	echo "Participant ${p} Interviews Rejected (${num_rejected} Total) - " >> "$repo_root"/audio_lab_email_body.txt
