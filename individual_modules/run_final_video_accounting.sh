...
echo "Beginning video file accounting script for study ${study}"
# will make one video accounting sheet per patient and interview type for this site
...
for p in *; do
	# first check that it is truly a patient ID, that has a video_frames folder for the interview type
	...
	python "$func_root"/interview_video_process_account.py "open" "$data_root" "$study" "$p"
...
for p in *; do
	...
	# compile metadata info as needed
	python "$func_root"/interview_video_process_account.py "psychs" "$data_root" "$study" "$p"
