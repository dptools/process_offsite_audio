...
if [ $auto_send_limit_bool = "Y" ] || [ $auto_send_limit_bool = "y" ]; then
	python "$func_root"/offsite_audio_length_check.py "$data_root" "$study" "$auto_send_limit"
...
for p in *; do # loop over all patients in the specified study folder
	# first check that it is truly an OLID, that has offsite audio data to send
	if [[ ! -d $p/offsite_interview/processed/audio_to_send ]]; then # check for to_send folder
		continue
	fi
	...
	python "$func_root"/offsite_transcribeme_sftp_push.py "$data_root" "$study" "$p" "$transcribeme_username" "$transcribeme_password" "$pipeline"
	...
	if [ -z "$(ls -A audio_to_send)" ]; then
		...
		echo ""
   		echo "Warning: some interviews meant to be pushed to TranscribeMe failed to upload. Check ${data_root}/PROTECTED/${study}/${p}/offsite_interview/processed/audio_to_send for more info."
   		echo ""
	fi
