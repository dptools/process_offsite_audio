...
if [[ -z ${mail_attach} ]]; then 
	...
	if [[ -e all-dists-by-type.pdf ]]; then
		mail -s "Weekly Pronet Interviews AVL QC Compilation" -A key-stats-table-by-site-and-type.html -A all-dists-by-type.pdf -A open-dists-by-site.pdf -A psychs-dists-by-site.pdf -A extended-counts-table.html -A combined-QC.csv "$deid_email_list" < dummy.txt
	elif [[ -e key-stats-table-by-site-and-type.html ]]; then
		mail -s "Weekly Pronet Interviews AVL QC Compilation" -A key-stats-table-by-site-and-type.html -A extended-counts-table.html -A combined-QC.csv "$deid_email_list" < dummy.txt
	else
	...
	if [[ -e all-dists-by-type.pdf ]]; then
		mail -s "Weekly Prescient Interviews AVL QC Compilation" -a key-stats-table-by-site-and-type.html -a all-dists-by-type.pdf -a open-dists-by-site.pdf -a psychs-dists-by-site.pdf -a extended-counts-table.html -a combined-QC.csv "$deid_email_list" < dummy.txt
	elif [[ -e key-stats-table-by-site-and-type.html ]]; then
		mail -s "Weekly Prescient Interviews AVL QC Compilation" -a key-stats-table-by-site-and-type.html -a extended-counts-table.html -a combined-QC.csv "$deid_email_list" < dummy.txt
	else
