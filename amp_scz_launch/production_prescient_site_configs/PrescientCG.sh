...
site_email_list="sophie.tod@orygen.org.au,isabelle.scott@orygen.org.au,beau-luke.colton@orygen.org.au,lana.kambeitz-ilankovic@uk-koeln.de,ana.muthesius@uk-koeln.de,susanne.inanloo-shavikloo@uk-koeln.de"
...
# only password needed for now is the one for TranscribeMe SFTP account
# transcribeme_password="password"
# export transcribeme_password
# however for prescient will also need to add the mediaflux account password as on prescient this repo is responsible for the manual site transcription review push
# the username will be specified in a separate utility bash script where the push runs (it is not part of the core pipeline), but keep passwords together for simplicity
